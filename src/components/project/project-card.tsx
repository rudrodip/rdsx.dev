...
export default function ProjectCard({ project }: ProjectCardProps) {
  return (
    <div className="flex p-3 justify-between gap-2 rounded-2xl border border-b-8 overflow-hidden">
      ...
      <Link href={`/projects/${project.slugAsParams}`} className="inline-flex items-center gap-1 group/link">
        ...
        <div className="flex items-center gap-2">
          {project.tags.map((tag) => (
            <p key={tag} className="px-2 py-1 rounded-lg bg-muted text-xs cursor-pointer">{tag}</p>
          ))}
        </div>
        <div className="space-x-2 h-10">
          {project.links.map((link) => (
            <a key={link.name} href={link.url} target="_blank" className="p-2 text-xs inline-flex gap-1 items-center rounded-lg pebble">
              <span>{link.name}</span>
