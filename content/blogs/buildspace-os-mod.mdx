...
But I was getting an error, `Refused to execute inline script because it violates the following Content Security Policy directive: "script-src 'self' 'unsafe-eval'".`. I asked chatgpt to fix it, but it disappointed me (foreshadowing hour long time waste)

First of all, it said "you can't modify the html of a iframe cz it comes from different origin". I was like, "alright, then whats the solution?"

It told me to make another html that loads the iframe, and the previous html file will load the new local html file. I was like, "alright, let's do it".

Did it work? No, it didn't. I was getting the same error.

So, I asked chatgpt again, and it said, "there is a much complicated way to achieve this. you can pass the message from the content script to the background script, and then from the background script to the content script of the iframe". I was like, "alright, let's do it".
...
<video className="w-full" controls src="https://github.com/rudrodip/buildspace-os-mod/assets/77154365/1fd7173c-a79e-4113-bab9-2f7a10213bc5" />
